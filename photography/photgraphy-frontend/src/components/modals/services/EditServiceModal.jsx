...
export default function EditServiceModal({
  ...
  const initialValues = {
    ...
    // date: convertDate(selectedService?.date) || "",
  ...
  useEffect(() => {
    if (selectedService && selectedService.images) {
      ...
      setFiles(
        selectedService.images.map((path, index) => ({
          path,
          id: index,
        }))
      );
  ...
  const handleUpdate = async (values, actions) => {
    try {
      ...
      // Update the portfolios state by replacing the updated service in the array
      ...
      handleSuccess(
        response.data.message || "You have successfully updated the service"
      );
      setOpen(false);
    } catch (error) {
